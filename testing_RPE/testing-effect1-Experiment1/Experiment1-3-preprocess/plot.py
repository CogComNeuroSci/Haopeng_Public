...
## import the data
data = pd.read_csv('data_preprocess_higher_than_0.34.csv')
...
data_plot = data_plot.groupby(by=['participant', 'reward2', 'reward3', 'srpe', 'learning_method'])[['pre-learning', 'accuracy']].mean()
data_plot = data_plot.reset_index()
data_plot.columns = ['Participant', 'Feedback(Phase2)', 'Feedback', 'RPE', 'Testing vs Studying', 'Pre-learning', 'Accuracy']
data_plot['Pure accuracy'] = data_plot['Accuracy'] - data_plot['Pre-learning']

# plot
fig = sns.catplot(data=data_plot, x='RPE', y='Accuracy', kind='bar', hue='Testing vs Studying', col='Feedback', sharex=False)
fig = sns.catplot(data=data_plot, x='RPE', y='Pre-learning', kind='bar', hue='Testing vs Studying', col='Feedback', sharex=False)
fig = sns.catplot(data=data_plot, x='RPE', y='Pure accuracy', kind='bar', hue='Testing vs Studying', col='Feedback', sharex=False)
fig.set(ylim=[0, 1])
