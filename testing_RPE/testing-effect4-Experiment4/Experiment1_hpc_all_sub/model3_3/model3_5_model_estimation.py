...
#for sub in [1]:
    ...
    # formal learning
    W3 = model_learning(b=parameters[0], slope=parameters[1], bias=parameters[2], W=W0.copy(), X=X_phase3, Y_feed=Y_feed_phase3, Y_options=Y_options_phase3, TvS=TvS_phase3, Reward3=Reward3_phase3, swa_words=swa_words)
    
    # model testing
    Y_pred = model_testing(slope=parameters[1], bias=parameters[2], W=W3.copy(), X=X_testing, Y_options=Y_options_testing, swa_words=swa_words)
...
parameters_all.columns = ['index', 'b', 'slope', 'bias', 'log_like', 'AIC', 'success', 'Pars']
parameters_all = parameters_all[['b', 'slope', 'bias', 'log_like', 'AIC', 'Pars']]

data_all = data_all.loc[data_all['Reward2']==0, :]
data_all = data_all.merge(parameters_all, how='left')

data_all = data_all.groupby(by=['Pars', 'b', 'slope', 'bias', 'TvS'])[['Model_accuracy', 'Human_accuracy']].mean()
data_all = data_all.reset_index()

data_all = data_all.pivot(columns=['TvS'], index=['Pars', 'b', 'slope', 'bias'], values=['Model_accuracy', 'Human_accuracy'])
data_all = data_all.reset_index()

data_all.columns = ['Pars', 'b', 'slope', 'bias', 'Model_accuracy_study', 'Model_accuracy_test', 'Human_accuracy_study', 'Human_accuracy_test']
