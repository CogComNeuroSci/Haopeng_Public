...
def model_learning(b1, b2, slope, bias, W, X, Y_feed, Y_options, TvS, Reward3, swa_words):
    ...
    TvS_3d = TvS.reshape(TvS.shape[0], 1, 1)
    # regular Hebbian learning
    W_update_3d = b1 * X_3d * Y_feed_3d
    W_update = W_update_3d.sum(axis=0)
    W += W_update
    # rewarded Hebbian kearning in correct test
    W_update_3d = b2 * X_3d * Y_feed_3d * Reward3_3d * TvS_3d
    W_update = W_update_3d.sum(axis=0)
    W += W_update
    # punished Hebbian learning in incorrect test
    Y_pred_3d = Y_pred.reshape([Y_pred.shape[0], 1, Y_pred.shape[1]])
    W_update_3d = -b2 * X_3d * Y_pred_3d * Reward3_3d
    W_update = W_update_3d.sum(axis=0)
    W += W_update
    # Delta learning
    #Y_pred_3d = Y_pred.reshape([Y_pred.shape[0], 1, Y_pred.shape[1]])
