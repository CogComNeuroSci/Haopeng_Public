...
def log_like(parameters, W0, X_phase2, Y_conf_phase2,
    ...
    #parameters[0:1] = np.exp(parameters[0:1])
    #parameters[0:1] = parameters[0:1]/parameters[0:1].sum()
    parameters[0:1] = 1
    parameters[1:3] = 10*logistic(parameters[1:3])
    #print(parameters)
    ...
    ### prelearning (phase 3)
    #W2 = model_prelearning(a=parameters[0], W=W0.copy(), X=X_phase3.copy(), Y_conf=Y_conf_phase3.copy())
    
    
    ### formal learning
    W3 = model_learning(b=parameters[0], slope=parameters[1], bias=parameters[2], W=W0.copy(), X=X_phase3.copy(), Y_feed=Y_feed_phase3.copy(), Y_options=Y_options_phase3.copy(), TvS=TvS_phase3.copy(), Reward3=Reward3_phase3.copy(), swa_words=swa_words.copy())
    
    
    ### model testing
    Y_pred = model_testing(slope=parameters[1], bias=parameters[2], W=W3.copy(), X=X_testing.copy(), Y_options=Y_options_testing.copy(), swa_words=swa_words.copy())
