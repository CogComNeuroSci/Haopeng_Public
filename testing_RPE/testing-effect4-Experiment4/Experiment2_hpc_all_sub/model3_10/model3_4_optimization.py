...
optimization: Pre-learning, Hebbian learning
"""
...
#for sub in [0]:
    ...
    for opt in range(10):
        ...
        # initial parameters
        x0 = random.choices([-1, 1], k=5)*np.random.rand(5)*5
        ...
        parameters[2] = 1*logistic(optimals[2])
        parameters[3:5] = 10*logistic(optimals[3:5])
    ...
    data_best = data_sub[data_sub.iloc[:, 6]==data_sub.iloc[:, 6].min()].iloc[0, :].to_frame().T
