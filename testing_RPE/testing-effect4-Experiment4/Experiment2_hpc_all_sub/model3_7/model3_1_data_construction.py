...
def data_construction(data_origin='', phases=[2, 3], file_path=''):
    ...
    AccuraciesB = np.ndarray([participants.shape[0], 90*len(phases), 1])*0
    AccuraciesC = np.ndarray([participants.shape[0], 90*len(phases), 1])*0
    InAccBs = np.ndarray([participants.shape[0], 90*len(phases), 1])*0
    InAccCs = np.ndarray([participants.shape[0], 90*len(phases), 1])*0
    ...
    for dim1 in range(W.shape[0]):
        ...
        for phase in phases:
            ...
            if phase == phases[0]:
                ...
                compare_accuracies_b = data_par['accuracy_binary'].values[:, np.newaxis]
                compare_accuracies_c = data_par['accuracy_continuous'].values[:, np.newaxis]
                compare_inaccbs = data_par['individual_binary_acc'].values[:, np.newaxis]
                compare_inacccs = data_par['individual_continuous_acc'].values[:, np.newaxis]
                ...
                compare_accuracy_b = data_par['accuracy_binary'].values[:, np.newaxis]
                compare_accuracies_b = np.concatenate([compare_accuracies_b, compare_accuracy_b], axis=0)
                
                compare_accuracy_c = data_par['accuracy_continuous'].values[:, np.newaxis]
                compare_accuracies_c = np.concatenate([compare_accuracies_c, compare_accuracy_c], axis=0)
                
                compare_inaccb = data_par['individual_binary_acc'].values[:, np.newaxis]
                compare_inaccbs = np.concatenate([compare_inaccbs, compare_inaccb], axis=0)
                
                compare_inaccc = data_par['individual_continuous_acc'].values[:, np.newaxis]
                compare_inacccs = np.concatenate([compare_inacccs, compare_inaccc], axis=0)
        ...
        AccuraciesB[dim1, 0:compare_accuracies_b.shape[0], :] = compare_accuracies_b
        AccuraciesC[dim1, 0:compare_accuracies_c.shape[0], :] = compare_accuracies_c
        InAccBs[dim1, 0:compare_inaccbs.shape[0], :] = compare_inaccbs
        InAccCs[dim1, 0:compare_inacccs.shape[0], :] = compare_inacccs
    ...
    np.save(file_path+'AccuraciesB.npy', AccuraciesB)
    np.save(file_path+'AccuraciesC.npy', AccuraciesC)
    np.save(file_path+'InAccBs.npy', InAccBs)
    np.save(file_path+'InAccCs.npy', InAccCs)
...
data_origin = 'data/E1_original_data/data_preprocess.csv'
