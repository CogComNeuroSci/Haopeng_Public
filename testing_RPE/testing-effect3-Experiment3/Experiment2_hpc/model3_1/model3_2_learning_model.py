...
def model_learning(b1, b2, slope, bias, W, X, Y_feed, Y_options, TvS, Reward3, swa_words):
    ### Options 
    ...
    W_update_3d = b1 * X_3d * Y_feed_3d
    W_update = W_update_3d.sum(axis=0)
    W += W_update
    # Delta learning
    Y_pred_3d = Y_pred.reshape([Y_pred.shape[0], 1, Y_pred.shape[1]])
    W_update_3d = b2 * X_3d * (Y_feed_3d - Y_pred_3d)
    W_update = W_update_3d.sum(axis=0)
    W += W_update
...
slope = 1
bias = 1
X1 = np.load('data/E1_data_construction/data_phase4/X.npy')
X2 = np.load('data/E1_data_construction/data_phase5/X.npy')
X = np.concatenate([X1, X2], axis=1)

Y_options1 = np.load('data/E1_data_construction/data_phase4/Y_options.npy')
Y_options2 = np.load('data/E1_data_construction/data_phase5/Y_options.npy')
Y_options = np.concatenate([Y_options1, Y_options2], axis=1)
